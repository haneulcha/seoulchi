...
**`24시간` / `상시개방` 계열만 처리해도 성공률이 눈에 띄게 오릅니다.** 나머지는 원문 노출이 정답입니다.
규칙 확장 여부는 미결정 — 현재는 스펙의 best-effort 원칙대로 `null` + 원문 노출로 동작합니다.
...
| 6 | 영업시간 파싱 성공률 | ✅ **54%** (표본 116건 중 63건). 규칙 확장 여부는 미결정 |
