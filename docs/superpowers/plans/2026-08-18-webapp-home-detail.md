...
- **#3 — 해소됨. 정적 서버 함수는 입력값별로 캐시된다. 대비책(`<Link>` → `<a href>`)은 적용하지 않았고, `src/components/cards.tsx`는 `<Link>` 그대로 둔다.** 근거 넷:
  ...
  **정정 (2026-08-19, `base: '/seoulchi/'` 도입 뒤).** 근거 1의 *"쓰기와 읽기가 같은 함수를 쓰므로 정의상 일치한다"* 는 **`base`가 `/`일 때만** 성립한다. 같은 문자열을 쓰기는 **출력 디렉터리 기준 상대 경로**(`path.join(TSS_CLIENT_OUTPUT_DIR, url)`)로, 읽기는 **origin 기준 절대 경로**(`fetch(url)`)로 해석하기 때문이다. GitHub Pages 프로젝트 사이트에서는 읽기 쪽만 어긋나 `/__tsr/…json`을 받으러 갔다가 Pages의 404 HTML을 JSON으로 파싱하고 앱이 터졌다 — 홈↔상세 클라이언트 네비게이션 전부. 근거 4의 정적 서버 검증이 이걸 못 잡은 이유도 같다: `dist/client`를 **서버 루트에** 얹어 돌렸으므로 `base`가 관여하지 않았다. `src/build/static-fn-base.ts`가 클라이언트 번들에만 base를 주입해 고쳤고, 이후 정적 서빙 검증은 **`<루트>/seoulchi/` 아래에 얹어서** 한다.
